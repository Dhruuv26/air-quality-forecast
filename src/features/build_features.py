...
def load_and_pivot_cpcb(path: str = f"{RAW_DIR}/cpcb_history.csv") -> pd.DataFrame:
    ...
    df = pd.read_csv(path)
    # real CPCB API returns DD-MM-YYYY HH:MM:SS; synthetic data uses ISO
    # (YYYY-MM-DDTHH:MM:SS). Parsing without an explicit format is ambiguous
    # for day<=12 (e.g. 03-04-2026 could be Mar 4 or Apr 3) — try the real
    # API's format first, fall back to ISO for synthetic/generated data.
    try:
